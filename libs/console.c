...
/******************************************************************************/
/**< console functions */
int console_set_size(HANDLE hnd, short xsize, short ysize)
{
  COORD console_sz;
  console_sz.X = xsize;
  console_sz.Y = ysize;
  return SetConsoleScreenBufferSize(hnd, console_sz);
}
int console_set_canvasSize(HANDLE hnd, short xsize, short ysize)
{
    SMALL_RECT canvas_sz = {0};
    canvas_sz.Right = xsize;
    canvas_sz.Bottom = ysize;
    return SetConsoleWindowInfo(hnd, 1, &canvas_sz);
